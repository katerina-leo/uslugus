...
export  const renderModal = (parent, data, closeModal) => {
    ...
    const service = createElement('div', {
        className: 'person__service service service--person'
    }, container);
    ...
    createElement('a', {
        className: 'service__link service__link--phone',
        textContent: data.phone,
        href: `tel:${data.phone}`,
    }, serviceContacts);

    createElement('a', {
        className: 'service__link service__link--email',
   ...
   createElement('h3', {
    ...
    if (data.comments.length) {
        ...
        if (data.comments.length > 3) {
            const btn = createElement('button', {
                className: 'review__open review__open--list',
                textContent: 'Все отзывы',
            }, review);

            btn.addEventListener('click', () => {
                review.classList.add('review--show-all');
                btn.remove();
            })
        }
    ...
    const wrapperAddReview = createElement('div', {
        className: 'form__wrapper-add-review',
    }, formReview);
    ...
    for (let i = 1; i <= 5; i++) {
        rating.innerHTML += `<svg class="rating__star" width="18" height="18" viewBox="0 0 18 18" data-rating="${i}" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
